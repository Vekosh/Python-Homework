...
GENDER, PHOTO, ID, LOCATION, BIO = range(5)

# функция обратного вызова точки входа в разговор
def start(update, _):
    # Список кнопок для ответа
    reply_keyboard = [['Парень', 'Девушка', 'Не скажу']]
...
def photo(update, _):
    ...
    update.message.reply_text(
        'Великолепно! А теперь давай узнаем'
        ' ID, или /skip если параноик..'
    )
...
def skip_photo(update, _):
    ...
    update.message.reply_text(
        'Держу пари, ты выглядишь великолепно! А теперь не хочешь'
        ' узнать свой ID, или /skip если параноик.'
    )
    return ID
...
def id(update,_):
    ...
    if update.message.chat.id == 'Да':
        update.message.reply_text(" %s ваш id. %s", user.first_name, user_id)
    else:
        logger.info("Пользователь %s указал свой id. %s", user.first_name, user_id)
        #update.message.reply_text('Великолепно!  Ваш id: %s', user_id) #Почему не работает? ERROR - No error handlers are registered, logging exception.
        update.message.reply_text(' Не хотите указать своё местоположение, или /skip если параноик..')
    return LOCATION

def skip_id(update, _):
    # определяем пользователя
    user = update.message.from_user
    # Пишем в журнал сведения о фото
    logger.info("Пользователь %s не отправил фото.", user.first_name)
    # Отвечаем на сообщение с пропущенной фотографией
    update.message.reply_text(
        'Ну и ладно! А теперь пришлите мне'
...
if __name__ == '__main__':
    ...
    updater = Updater("5647985044:AAEHlXf1p4e5j2QWjhrS_vcbxdoAI4lNHKY")
    ...
    conv_handler = ConversationHandler( # здесь строится логика разговора
        ...
        states={
            GENDER: [MessageHandler(Filters.regex('^(Парень|Девушка|Не скажу)$'), gender)],
            PHOTO: [MessageHandler(Filters.photo, photo), CommandHandler('skip', skip_photo)],
            LOCATION: [
                MessageHandler(Filters.location, location),
                CommandHandler('skip', skip_location),
            ],
            ID: [MessageHandler(Filters.text, id), CommandHandler('skip', skip_id)],
